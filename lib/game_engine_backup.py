...
class game_engine:
    ...
    def __init__(self, _map, parcel_gen_gap, parcel_gen_seq=None, step_left=None, auto_unload=False):
        """
    ...
    def step(self, moves):
        ...
        tmp = np.array([[i,move[0],move[1],move[2]] for i,move in enumerate(moves)])
        tmp = np.random.permutation(moves_indices)

        tmp = build_topology(tmp)
        moves_indices = topological_sort(tmp)
